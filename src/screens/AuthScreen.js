...
export default function AuthScreen() {
  ...
  // ─── GOOGLE SIGN IN (existing account only) ───
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (!exists) {
        // User hasn't signed up before - delete the auth account and show error
      ...
      // User exists, sign in successful - auth state listener handles the rest
    } catch (e) {
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      if (exists) {
        setError('Account already exists with this Google account. Try signing in.');
        setLoading(false);
        return;
      }

      // Create user doc with Google profile data
      ...
      // Sign up successful - auth state listener handles the rest
    } catch (e) {
  ...
  return (
    <div style={{
      ...
      <p style={{
        ...
        letterSpacing: 0.5,
        fontWeight: 400
      }}>Your personal lighting studio</p>
      ...
      <div style={{ width: '100%', maxWidth: 380 }}>
        ...
        {/* Google Button */}
        <button
          onClick={tab === 'signin' ? handleGoogleSignIn : handleGoogleSignUp}
          disabled={loading}
          style={{
            width: '100%',
            padding: 15,
        ...
        <div>
          ...
          <div style={{ ...inputWrapStyle }}>
            <input
              ...
              onKeyDown={e => {
                if (e.key === 'Enter') {
                  if (tab === 'signin') handleEmailSignIn();
                }
              }}
            />
            <button
          ...
          {tab === 'signup' && (
            <div style={{ ...inputWrapStyle }}>
              ...
              >
                {showConfirmPass ? <HiEyeSlash size={18} /> : <HiEye size={18} />}
              </button>
            </div>
          )}
          ...
          {tab === 'signup' && pass.length > 0 && (
            <div style={{
              ...
              {[1, 2, 3, 4].map(i => {
                ...
                const colors = ['#ff4444', '#ffaa00', '#ffdd00', '#22c55e'];
                return (
                  <div key={i} style={{
                    flex: 1,
                    height: 3,
          ...
          {error && (
            <div style={{
              ...
              <HiExclamationCircle size={18} color="#ff4444" style={{ flexShrink: 0, marginTop: 1 }} />
              <span style={{
                color: '#ff6666',
                fontSize: 13,
                fontWeight: 500,
